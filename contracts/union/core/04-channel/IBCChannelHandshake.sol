...
contract IBCChannelHandshake is ModuleManager, IIBCChannelHandshake {
    ...
    function channelOpenInit(
        ...
        commitments[
            IBCCommitment.nextSequenceSendCommitmentKey(msg_.portId, channelId)
        ] = bytes32(uint256(1));
        commitments[
            IBCCommitment.nextSequenceRecvCommitmentKey(msg_.portId, channelId)
        ] = bytes32(uint256(1));

        commitments[
            IBCCommitment.nextSequenceAckCommitmentKey(msg_.portId, channelId)
        ] = bytes32(uint256(1));
    ...
    function generateChannelIdentifier() internal returns (string memory) {
        uint256 nextChannelSequence = uint256(
    ...
    function ensureConnectionState(
        string memory connectionId
    ) internal view returns (IbcCoreConnectionV1ConnectionEnd.Data memory) {
